...
IMPLEMENTED_CASES: tuple[FourierCase, ...] = (
    ...
    FourierCase("B06", "u(t)", "basic", ("pi", "delta", "omega"), expected_result_latex=r"\pi \delta\left(\omega\right) - j (\mathrm{PV})\frac{1}{\omega}"),
    FourierCase("B07", "u(t-2)", "basic", ("e", "2", "omega", "delta"), expected_result_latex=r"\left(\pi \delta\left(\omega\right) - j (\mathrm{PV})\frac{1}{\omega}\right) e^{- 2 j \omega}"),
    FourierCase("B08", "u(t+1)", "basic", ("e", "omega", "delta"), expected_result_latex=r"\left(\pi \delta\left(\omega\right) - j (\mathrm{PV})\frac{1}{\omega}\right) e^{j \omega}"),
    ...
    FourierCase("PU05", "(t^2+2*t+1)*u(t)", "polynomial_step", ("PV", "omega"), expected_result_latex=r"\left(\pi \delta\left(\omega\right) - j (\mathrm{PV})\frac{1}{\omega}\right) - \left(\pi \delta^{(2)}(\omega) - 2 j (\mathrm{PV})\frac{1}{\omega^{3}}\right) + \left(2 j \pi \delta^{(1)}(\omega) - 2 (\mathrm{PV})\frac{1}{\omega^{2}}\right)"),
    FourierCase(
        "PU06",
        "(t-2)*u(t-2)",
