...
IMPLEMENTED_CASES: tuple[FourierCase, ...] = (
    ...
    FourierCase("PU05", "(t^2+2*t+1)*u(t)", "polynomial_step", ("PV", "omega"), expected_result_latex=r"\left(\pi \delta\left(\omega\right) - \frac{j}{\omega}\right) - \left(\pi \delta^{(2)}(\omega) - 2 j \mathrm{PV}\!\left(\frac{1}{\omega^{3}}\right)\right) + \left(2 j \pi \delta^{(1)}(\omega) - 2 \mathrm{PV}\!\left(\frac{1}{\omega^{2}}\right)\right)"),
    FourierCase(
        "PU06",
        "(t-2)*u(t-2)",
